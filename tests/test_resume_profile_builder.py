"""
Failing test stubs (RED phase) for ResumeProfileBuilder behavioral contracts.
...
These tests WILL fail until Plans 02 and 04 implement the production code.
"""
...
def test_single_occurrence_token_excluded() -> None:
    ...
    config: dict[str, Any] = _make_config()
    builder: ResumeProfileBuilder = ResumeProfileBuilder()
    # _load_taxonomy does not exist yet (Plan 04); create=True allows forward-patching
    taxonomy: frozenset[str] = frozenset()  # Widgetlang not in taxonomy
    with patch("application.resume_profile_builder.ResumeProfileBuilder._load_config", return_value=config), \
         patch("application.resume_profile_builder._load_taxonomy", return_value=taxonomy, create=True):
        profile = builder.build(_RESUME_WITH_ONE_OCCURRENCE)
    # Plan 04 adds occurrence threshold; until then this assertion drives the RED state
