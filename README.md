...
- Scanning the **Experience** and **Projects** sections for capitalized tokens, then applying two filters: (1) **frequency gate** — tokens appearing only once are dropped; (2) **taxonomy gate** — tokens are checked against `infrastructure/tech_taxonomy.yaml` (60 curated tech terms). Taxonomy hits are always included. Unknown tokens are optionally classified by Gemini (when `GEMINI_API_KEY` is set) and cached in `infrastructure/tertiary_cache.json`; without a key, unknown tokens are kept (fail-open). Survivors become `tertiary_skills` with weight **1**. Generic or frontend-only terms not relevant to backend scoring (e.g. `rest`, `api`, `html`, `github`) are excluded via a stop-word list.
...
6. Self-review: re-read its own changes, fix every issue found (add missing tests, correct comments, remove taxonomy conflicts, add type annotations, revert speculative or collateral-damage rules), commit the corrections, and post a PR comment summarizing what was found and what was done
...
7. Self-review: re-read its own changes, fix every issue found (add missing tests, add type annotations, move misplaced I/O, revert fixes that don't match the `last_error` or lack live endpoint evidence), commit the corrections, and post a PR comment summarizing what was found and what was done
