...
6. Self-review: re-read its own changes and post a PR comment checking for missing tests, inaccurate comments, taxonomy/stop-word conflicts, missing type annotations, business rule compliance, and whether each change is backed by real evidence in `jobs_debug.json` (no speculative rules, no collateral damage on legitimate titles)
...
7. Self-review: re-read its own changes and post a PR comment checking for missing test coverage, missing type annotations, architecture violations (I/O in domain/application), business rule compliance, and whether the fix directly addresses the `last_error` with live endpoint evidence (no speculative changes, no unintended behavior alterations)
